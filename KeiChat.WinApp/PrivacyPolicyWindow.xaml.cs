...
using KeiChat.Common.Utilities;

namespace KeiChat.WinApp
{
    /// <summary>
    /// PrivacyPolicyWindow.xaml 的交互逻辑
    /// </summary>
    public partial class PrivacyPolicyWindow : Window
    {
        private static PrivacyPolicyWindow? _instance;
        // 当前窗口只允许打开一个。
        private static readonly AtomicBoolean _atomicBoolean = new AtomicBoolean();

        internal static void Open()
        {
            if (_atomicBoolean.CompareAndSet(false, true))
            {
                _instance = new PrivacyPolicyWindow();
                _instance.Show();
            }
